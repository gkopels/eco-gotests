...
// NewMachineConfigurationBuilder creates a new instance of machineconfiguration_remove.
...
// Pull pulls existing machineconfiguration_remove from cluster.
func Pull(apiClient *clients.Settings, name string) (b *MCBuilder, err error) {
	...
	if !builder.Exists() {
		return nil, fmt.Errorf("machineconfguration object %s does not exist", name)
	}
...
// GetMachineConfigurationGVR returns machineconfiguration_remove's GroupVersionResource which could be used for Clean function.
...
func (builder *MCBuilder) Update(force bool) (*MCBuilder, error) {
	...
	if !builder.Exists() {
		return nil, fmt.Errorf("machineconfiguration object does not exist")
	}

	mcUpdating, err := builder.Get()
	if err != nil {
		return nil, err
	}

	fmt.Println("mcUpdating", mcUpdating.Spec)

	fmt.Println("REVISION", builder.Definition.ResourceVersion)
	fmt.Println("DEFINITION", builder.Definition)
...
// Exists checks whether the given machineconfiguration_remove exists.
...
func (builder *MCBuilder) Delete() (*MCBuilder, error) {
	...
	if err != nil {
		return builder, fmt.Errorf("can not delete machineconfiguration_remove: %w", err)
	}
...
func (builder *MCBuilder) WithNodeDisruptionPolicy(name, path string) *MCBuilder {
	...
	builder.Definition.Spec.NodeDisruptionPolicy = machineconfigurationv1.NodeDisruptionPolicyConfig{
		...
		SSHKey: machineconfigurationv1.NodeDisruptionPolicySpecSSHKey{
			Actions: []machineconfigurationv1.NodeDisruptionPolicySpecAction{
				{
					Type: machineconfigurationv1.NoneSpecAction,
				},
			},
		},
	}
