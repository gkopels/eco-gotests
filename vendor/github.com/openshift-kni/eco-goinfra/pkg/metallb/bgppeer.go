...
// WithConnectTimer defines the reconnect timer between BGP neighbors.
func (builder *BGPPeerBuilder) WithConnectTimer(connectTimer metav1.Duration) *BGPPeerBuilder {
	if valid, _ := builder.validate(); !valid {
		return builder
	}
	...
	glog.V(100).Infof(
		"Creating BGPPeer %s in namespace %s with this holdTime: %s",
		builder.Definition.Name, builder.Definition.Namespace, connectTimer)
	...
	builder.Definition.Spec.ConnectTime = &connectTimer
