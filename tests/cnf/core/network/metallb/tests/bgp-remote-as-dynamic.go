		...
		Context("single hop", func() {
			...
			It("Verify the establishment of an eBGP adjacency using neighbor peer remote-as external",
				reportxml.ID("76821"), func() {
					...
					Eventually(func() int {
						remoteAS, err := frr.FetchBGPRemoteAS(frrk8sPods, ipv4metalLbIPList[0])
						Expect(err).ToNot(HaveOccurred(), "Failed to verify BGP remote AS")

						return remoteAS
					}, 60*time.Second, 5*time.Second).Should(Equal(tsparams.RemoteBGPASN),
						fmt.Sprintf("The remoteASN doesnt not match the expected AS: %d", tsparams.RemoteBGPASN))
				})
			...
			It("Verify the establishment of an iBGP adjacency using neighbor peer remote-as internal",
				reportxml.ID("76822"), func() {
					...
					Eventually(func() int {
						...
						return remoteAS
					}, 60*time.Second, 5*time.Second).Should(Equal(tsparams.LocalBGPASN),
						fmt.Sprintf("The remoteASN doesnt not match the expected AS: %d", tsparams.LocalBGPASN))
				})
