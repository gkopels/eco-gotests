...
import (
	...
	"github.com/openshift-kni/eco-goinfra/pkg/machineconfiguration"
...
func CreateNftableCustomRule() *mco.MCBuilder {
...
data:;base64,`
	...
	_, err := json.Marshal(ignitionConfig)
	...
	//	WithRawConfig(finalIgnitionConfig).Create()

	testMC, err := mco.NewMCBuilder(APIClient, "98-nftables-cnf-worker").
		WithLabel("machineconfiguration.openshift.io/role", NetConfig.CnfMcpLabel).
		Create()
...
func AddNFTSvcToNodeDisruptionPolicy(apiClient *clients.Settings) error {
	machineConfigurationClient, err := apiClient.GetAPIClient()
	...
	err = machineConfigurationClient.Update(context.TODO(), types.Object(applyConfiguration),
		metav1.ApplyOptions{FieldManager: "machine-config-operator", Force: true})

	OperatorV1().MachineConfigurations().Apply(context.TODO(), applyConfiguration,
		metav1.ApplyOptions{FieldManager: "machine-config-operator", Force: true})
	if err != nil {
