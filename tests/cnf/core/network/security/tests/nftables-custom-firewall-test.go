	...
	Context("custom firewall", func() {
		...
		It("Verify the creation of a new custom node firewall NFTables table with an ingress rule",
			reportxml.ID("77412"), func() {
				...
				err = validateTCPTraffic(masterPod, ipv4NodeAddrList, interfaceNet1, containerFrr)
...
func updateMachineConfigurationCluster(nftables bool) {
	...
	err = WaitForMcpStable(APIClient, 35*time.Minute, 1*time.Minute, "workercnf")
...
func setupRemoteMultiHopTest(ipv4SecurityIPList, hubIPv4ExternalAddresses,
	...
	hub1BRstaticIPAnnotation := createStaticIPAnnotations(tsparams.ExternalMacVlanNADName, nadHub,
	...
	masterStaticIPAnnotation := createStaticIPAnnotations(tsparams.ExternalMacVlanNADName, nadHub,
...
func createMasterPodTest(name, nodeName, configmapName string, defaultCMD []string,
	...
	frrContainer := pod.NewContainerBuilder(
		containerFrr, NetConfig.CnfNetTestContainer, []string{"/bin/bash", "-c",
