...
var _ = Describe("nftables", Ordered, Label(tsparams.LabelNftablesTestCases), ContinueOnFailure, func() {
	...
	BeforeAll(func() {
		...
		testPodWorker0 = createTestPodsOnWorkers("testpod1-", workerNodeList[0].Definition.Name, 8888)
		testPodWorker1 = createTestPodsOnWorkers("testpod2-", workerNodeList[1].Definition.Name, 8888)
		//_ = createTestPodsOnWorkers("testpod3-", workerNodeList[0].Definition.Name, 8088)
		//_ = createTestPodsOnWorkers("testpod4-", workerNodeList[1].Definition.Name, 8088)
	...
	Context("custom firewall", func() {
		...
		It("Verify the creation of a new custom node firewall NFTables table with an ingress rule",
			reportxml.ID("77142"), func() {
				...
				masterPod := setupRemoteMultiHopTest(ipv4SecurityIPList, hubIPv4ExternalAddresses,
					ipv4NodeAddrList, workerNodeList,
					[]string{"hub-pod-worker-0", "hub-pod-worker-1"}, 8888)

				By("Verify ICMP connectivity between the master Pod and the test pods on the workers")
				err := cmd.ICMPConnectivityCheck(masterPod, ipv4NodeAddrList, interfaceNet1)
				...
				err = validateTCPTraffic(masterPod, ipv4NodeAddrList, 8888, interfaceNet1, containerFrr)
				Expect(err).ToNot(HaveOccurred(),
					"Failed to send TCP traffic over port 8888 to the worker nodes")

				By("Verify egress TCP traffic over port 8888 between the test Pod on the worker and the master pod")
				err = validateTCPTraffic(testPodWorker0, []string{masterPodIPv4Address},
					8888, interfaceBrEx, "")
				Expect(err).ToNot(HaveOccurred(),
					"Failed to send TCP traffic over port 8888 to the worker nodes")

				By("Define and create a NFTables custom rule blocking ingress TCP port 8888")
				createMCAndWaitforMCPStable(tsparams.CustomFirewallIngressPort8888)

				By("Verify ingress TCP traffic is blocked over port 8888 between the master Pod and the test" +
					" pods on the workers")
				err = validateTCPTraffic(masterPod, ipv4NodeAddrList, 8888, interfaceNet1, containerFrr)
				Expect(err).To(HaveOccurred(),
					"Successfully sent TCP traffic over port 8888 to the worker nodes")

				By("Verify egress TCP traffic over port 8888 between the test Pod on the worker and the master pod")
				err = validateTCPTraffic(testPodWorker0, []string{masterPodIPv4Address}, 8888,
					interfaceBrEx, "")
				Expect(err).ToNot(HaveOccurred(),
					"Failed to send TCP traffic over port 8888 to the worker nodes")
			})

		It("Verify a custom node firewall nftables table with a egress rule added to the ingress rule",
			reportxml.ID("77143"), func() {
				...
				By("Define and create a NFTables custom rule blocking both the ingress and egress port 8888")
				createMCAndWaitforMCPStable(tsparams.CustomFirewallIngressEgressPort8888)

				By("Verify ingress TCP traffic is blocked over port 8888 between the master Pod and the test" +
					" pods on the workers")
				err = validateTCPTraffic(masterPod, ipv4NodeAddrList, 8888, "net1", "frr")
				Expect(err).To(HaveOccurred(),
					"Successfully sent TCP traffic over port 8888 to the worker nodes")

				By("Verify egress TCP traffic is blocked over port 8888 between the test Pod on the worker and " +
					"the master pod")
				err = validateTCPTraffic(testPodWorker0, []string{masterPodIPv4Address},
					8888, interfaceBrEx, "")
				Expect(err).To(HaveOccurred(),
		...
		FIt("Verify a custom firewall nftables is reloaded after host reboot with all existing rules",
			reportxml.ID("77144"), func() {
				...
				By("Reboot node worker-0 and verify nftables is still functioning after reboot")
				rebootNode(workerNodeList[0].Definition.Name)
...
func updateMachineConfigurationCluster(nftables bool) {
	...
	err = WaitForMcpStable(APIClient, 10*time.Minute, 1*time.Minute, "workercnf")
...
func setupRemoteMultiHopTest(ipv4SecurityIPList, hubIPv4ExternalAddresses,
	ipv4NodeAddrList []string, workerNodeList []*nodes.Builder, hubPodWorkerName []string, portNum int) *pod.Builder {
	...
	configMapStaticRoutes := defineConfigMapWithStaticRouteAndNetwork(hubIPv4ExternalAddresses,
		removePrefixFromIPList(ipv4NodeAddrList))
...
func createHubConfigMap(name string) *configmap.Builder {
	...
	hubConfigMap, err := configmap.NewBuilder(APIClient, name, tsparams.TestNamespaceName).
...
func createFrrHubPod(name, nodeName, configmapName string, defaultCMD []string,
	...
	By("Creating FRR hub pod in the test namespace")
...
func createMasterPodTest(name, nodeName, configmapName string, defaultCMD []string,
	secondaryNetConfig []*types.NetworkSelectionElement, portNum int) *pod.Builder {
	frrPod := pod.NewBuilder(APIClient, name, tsparams.TestNamespaceName, NetConfig.FrrImage).
		DefineOnNode(nodeName).
		WithTolerationToMaster().
	...
	frrContainer := pod.NewContainerBuilder(
		"frr", NetConfig.CnfNetTestContainer, []string{"/bin/bash", "-c",
			fmt.Sprintf("testcmd -interface net1 -protocol tcp -port %d -listen", portNum)}).
		WithSecurityCapabilities([]string{"NET_ADMIN", "NET_RAW", "SYS_ADMIN"}, true)
...
func setWorkerNodeListAndLabelForTests(workerNodeList []*nodes.Builder) (map[string]string, []*nodes.Builder) {
...
func buildRoutesMapWithSpecificRoutes(podList []*pod.Builder, nextHopList []string) map[string]string {
	...
	Expect(len(nextHopList)).ToNot(BeZero(), "Next hop IP addresses list is empty")
...
func createTestPodsOnWorkers(name, nodeName string, portNum int) *pod.Builder {
	testPod, err := pod.NewBuilder(
		APIClient, name+nodeName, tsparams.TestNamespaceName, NetConfig.CnfNetTestContainer).
...
func validateTCPTraffic(clientPod *pod.Builder, destIPAddrs []string, portNum int, interfaceName, containerName string) error {
	for _, destIPAddr := range removePrefixFromIPList(destIPAddrs) {
		command := fmt.Sprintf("testcmd -interface %s -protocol tcp -port %d -server %s", interfaceName, portNum, destIPAddr)
...
func rebootNode(nodeName string) {

	By("Collecting information to create a BMC client")
	bmcClient, err := createBMCClient()
	Expect(err).ToNot(HaveOccurred(), "Failed to create BMC client")

	err = bmcClient.SystemPowerCycle()

	Expect(err).ToNot(HaveOccurred(), "Failed to reboot node")

	netenv.WaitForMcpStable(
		netinittools.APIClient, 35*time.Minute, 1*time.Minute, netinittools.NetConfig.CnfMcpLabel)
}
